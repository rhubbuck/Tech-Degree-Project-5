...
function searchFunction() { 
    ...
    let anchor = document.getElementsByClassName("disp");
    console.log(userInput);

//Loop through list items and hide the ones that don't match

    for (let i = 0; i < anchor.length; i++) {
        let x = anchor[i].getAttribute("data-caption").toLowerCase();
        if (!x.includes(userInput)) {
            anchor[i].style.display = "none";
        } else {
            console.log(anchor[i].getAttribute("data-caption").toLowerCase());
